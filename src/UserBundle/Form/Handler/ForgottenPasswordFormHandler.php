...
use UserBundle\Service\ForgottenPasswordKeyGenerator;
use UserBundle\Service\Mail\ResetPasswordLinkMailSender;
use UserBundle\Service\ResetUserPasswordUrlGenerator;
...
class ForgottenPasswordFormHandler extends FormHandler {
    ...
    public function handle(Request $request, Form $form)
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            try {
                $user = $this->em->getRepository('UserBundle:User')->findOneByUsernameOrEmail($userEmailOrUsername);
            } catch (\Exception $e) {
